...
\section{Discussion}
% =====================================================================
% OUTLINE -- expand each bullet below into prose, then delete the comments.
% Lead with the causal McNemar result, then corroboration, then validity,
% then answer the research question.
% =====================================================================
%
% THE ARGUMENT (in order):
% 1. HEADLINE / causal result -- the paired McNemar. The SAME questions, only the
%    figure/table format changes (paired text 64.2% vs paired image 44.8%,
%    p = 8.9e-5, a ~19-point penalty). Causal because difficulty is held fixed and
%    only the modality varies.
% 2. Gemma reads, it does not guess -- every format beats the 25% baseline (binomial),
%    so it extracts real information, but it only clears the 50% passing threshold on
%    the TEXT formats and fails on the image formats (the text-passes / image-fails split).
% 3. The unpaired text-vs-image z (73.8% vs 37.6%) points the same way but is
%    CONFOUNDED by question difficulty (different questions) -- correlation is not
%    causation; the McNemar is the causal anchor and the z only corroborates.
% 4. No contamination -- the clean post-cutoff 2025 exams scored HIGHER, not lower, so
%    memorised training data is not inflating the results; this strengthens the modality
%    finding.
% 5. Answer the research question -- yes, Gemma is significantly worse reading a
%    figure/table as an image than as text; figure/table questions are the more
%    AI-resistant format (ties back to the introduction).
%
% LIMITATIONS (where the rigour grade lives -- be honest):
% - Difficulty is confounded with modality in the unpaired comparison.
% - The paired set is mostly tables/matrices with few real figures -> the causal result
%   is really a TABLE-reading penalty; geometric figures appear only in the confounded z.
%   Do not overclaim "figures and tables" causally.
% - The text arm uses AI-generated (Fable 5) descriptions -> it partly tests description
%   quality, not modality alone.
% - Narrow scope: one course (02450 ML), one model at one size (Gemma-4-E2B).
% - The contamination check is weakly powered (only 2 clean exams, MDE ~24%); "no
%   contamination" is a non-rejection, not proof.
% - The 432 questions are treated as independent, but questions within one exam may
%   cluster by that exam's difficulty (the McNemar is immune; this affects the unpaired
%   z and the binomial tests).
%
% PRACTICAL TAKEAWAY:
% - Local open multimodal models handle text MCQs well but are markedly weaker on
%   figures/tables-as-images -> relevant for students using local models and for
%   assessment design (figure/table questions are more AI-resistant -> useful against
