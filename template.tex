...
One-sided binomial tests were ran on each input format against two baselines: the 25\% accuracy expected from guessing ($H_0: p = 0.25$), and the 50\% DTU passing threshold ($H_0: p = 0.50$). Within each family of four, p-values were compared to the Bonferroni-corrected $\alpha_{\text{Bonf}}=0.0125$. The same test was also applied to two full-exam simulations that combine the different modalities as in a realistic exam (bottom rows).

\begin{center}
\begin{tabular}{lccccc}
\toprule
\textbf{Input format} & \textbf{Accuracy} & \textbf{$p$ (vs.\ 25\%)} & \textbf{\shortstack{Better than\\ guessing}} & \textbf{$p$ (vs.\ 50\%)} & \textbf{Pass} \\
\midrule
\multicolumn{6}{@{}l}{\textit{By input format}} \\
