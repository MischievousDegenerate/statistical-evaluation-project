...
\begin{itemize}
    ...
    \item \textbf{Paired questions}: The question contains both visual representations and text needed in order to solve the question.
\end{itemize}
...
The outcome metric in all analyses is accuracy, the proportion of questions answered correctly. Because each question has exactly one correct option and is scored as a symmetric correct/incorrect outcome with no minority or rare-event class, accuracy is an unbiased measure of performance here, so the class-imbalance concerns that motivate metrics such as precision and recall do not apply.
...
where $\hat{p}$ is the observed accuracy and $n$ is the number of questions. The value $1.96$ is the $97.5\%$ quantile of the standard normal distribution and gives 95\% coverage, and the samples are large enough for the normal approximation to be reasonable.
...
The binomial tests form two families of four, with one test per input format. The first family assesses whether Gemma's accuracy exceeded the 25\% baseline expected under random guessing in a four-option multiple-choice setting. The second assesses whether Gemma could pass the exam, i.e. whether its accuracy exceeded the 50\% threshold. \newline 
Two further full-exam simulations test whether Gemma could pass a realistic exam containing both text and image questions. The first combines the unpaired text, unpaired image and paired-text questions; the second combines them with the paired questions as a screenshot.\newline
...
Since each family contains four binomial tests asking the same question, there is a risk that one of them returns
...
One-sided binomial tests were ran on each input format against two baselines: the 25\% accuracy expected from guessing ($H_0: p = 0.25$), and the 50\% DTU passing threshold ($H_0: p = 0.50$). Within each family of four, p-values were compared to the Bonferroni-corrected $\alpha_{\text{Bonf}}=0.0125$.
...
Every modality beats chance, all four $p$-values against the 25\% baseline are far below the corrected threshold. Against the 50\% passing threshold the formats split along modalities. Gemma passes when the information is text (unpaired text and paired text descriptions) but fails when the questions are presented as a screenshot.

To simulate a full exam, the same binomial test was used, but instead of unpaired text or unpaired image, the different modalities were combined. This simulated a full exam since normal exams are combinations of the different modalities.
...
\midrule
Unpaired text + unpaired image + paired (image)  & 51.6\% & $2.1 \times 10^{-32}$ & $0.27$               & No  \\
Unpaired text + unpaired image + paired (text)   & 57.6\% & $6.0 \times 10^{-47}$ & $8.7 \times 10^{-4}$ & Yes \\
\bottomrule
\end{tabular}
\end{center}
