...
Pure image, screenshot only (Type B)  & 52  & 144 & 36.1\% \\
\midrule
...
\begin{table}[ht]
\centering
\caption{Accuracy by group with 95\% Wilson score confidence intervals (in percentage points).}
...
Pure text (Type A)                         &  98 & 134 & 73.1\% & [65.1,\,79.9] \\
Paired question, text description (Type C) &  80 & 127 & 63.0\% & [54.3,\,70.9] \\
Paired question, screenshot (Type C)       &  54 & 127 & 42.5\% & [34.3,\,51.2] \\
Pure image, screenshot only (Type B)       &  52 & 144 & 36.1\% & [28.7,\,44.2] \\
\midrule
\multicolumn{5}{@{}l}{\textit{Paired questions --- asked both as an image and as text}} \\
...
Binomial & Paired text\_desc      & 63.0\% & $1.9 \times 10^{-19}$ & Yes \\
Binomial & Paired screenshot      & 42.5\% & $1.2 \times 10^{-5}$  & Yes \\
Binomial & Screenshot only        & 36.1\% & $2.0 \times 10^{-3}$  & Yes \\
\midrule
...
$\alpha_{\text{Bonf}} = 0.01$, so we can confidently say that Gemma performs 
above chance in every format. Even on the hardest group, image-only 
...
\subsection{Binomial tests: can Gemma pass the exam?}

Beyond testing whether Gemma performs above random guessing, a more practically relevant question is whether it would actually \textit{pass} the exam. The passing threshold at DTU is 50\%, so a one-sided binomial test against $H_0: p = 0.50$ was conducted for each input format, applying Bonferroni correction for four tests ($\alpha_\text{Bonf} = 0.0125$).

\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Input format} & \textbf{Accuracy} & \textbf{$p$-value} & \textbf{Passes?} \\
\midrule
Pure text              & 73.1\% & $4.5 \times 10^{-8}$ & Yes \\
Paired text\_desc      & 63.0\% & $1.7 \times 10^{-3}$ & Yes \\
Paired screenshot      & 42.5\% & $0.95$               & No  \\
Screenshot only        & 36.1\% & $> 0.99$             & No  \\
\bottomrule
\end{tabular}
\end{center}
...
The results split cleanly along modality lines. When information is provided as text, Gemma passes comfortably. When visual elements are presented as screenshots, performance falls well below the passing threshold with no statistical evidence of passing.

To simulate a realistic full exam, one modality was assigned per question as it would be in practice: Type A questions as text, Type B and Type C questions as screenshots. The results are shown below alongside the best-case scenario where text descriptions are provided for Type C questions instead of screenshots.

\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Scenario} & \textbf{Correct} & \textbf{Accuracy} & \textbf{$p$-value} \\
\midrule
Realistic (screenshots for B and C)       & 204/405 & 50.4\% & $0.44$ \\
Best case (text descriptions for C)       & 230/405 & 56.8\% & $3.0 \times 10^{-3}$ \\
\bottomrule
\end{tabular}
\end{center}
...
In the realistic scenario Gemma barely exceeds the passing threshold, but the result is not statistically significant ($p = 0.44$). In the best-case scenario, where table questions are provided as text descriptions, Gemma passes with $p = 0.003$. Whether Gemma reliably passes a full exam therefore depends entirely on whether the visual content is made available as text.
...
\vspace{5ex}
The contingency table summarizes the paired outcomes for 127 exam questions evaluated using both prompting methods for the Type C question. Both methods correctly answered 45 questions, while both methods incorrectly answered 38 questions.
