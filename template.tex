...
\begin{itemize}
    ...
    \item \textbf{Image}: The information required to answer the question is presented exclusively through a visual element such as a graph, figure, or diagram. These questions couldn't be translated into text directly.
    \end{itemize}
    
    \item \textbf{Paired questions}: The question contains information that can both be described as text and as a screenshot. e.g. questions containing a matrix, a vector or a table. 
\end{itemize}
...
The outcome metric in all analyses is accuracy, the proportion of questions answered correctly. Because each question has exactly one correct option, is scored as correct/incorrect and the data isn't imbalanced, accuracy is a good performance measure to use. This matters because accuracy can be misleading for imbalanced data, where precision and recall are preferred instead, as shown in slide~5 of the Day~4 lecture material\supercite{das2026day4}.
