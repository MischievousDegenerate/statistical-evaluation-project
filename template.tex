...
\begin{enumerate}
    \item \textbf{Headline causal result --- the paired McNemar test.} On the same questions, with only the figure/table format changed, Gemma scored 64.2\% when the element was given as text against 44.8\% when it was given as an image ($p = 8.9 \times 10^{-5}$), a $\sim$19-point penalty. This comparison is causal because the question is held fixed and only the modality varies.
    \item \textbf{Gemma reads, it does not guess.} Every format beats the 25\% chance baseline (binomial test), so the model extracts real information. However, it only clears the 50\% passing threshold on the text formats and fails on the image formats.
    \item \textbf{The unpaired text-vs-image comparison corroborates but is confounded.} The 73.8\% vs 37.6\% gap points the same way, but the two groups are different questions, so question difficulty is confounded with modality. The McNemar test is therefore the causal anchor and this z-test only supports it.
    \item \textbf{No data contamination.} The clean post-cutoff 2025 exams scored higher, not lower, so memorised training data is not inflating the results, which strengthens the modality finding.
    \item \textbf{Answer to the research question.} Gemma is significantly worse at reading a figure or table as an image than as text, so figure and table questions are the more AI-resistant format.
\end{enumerate}

\subsection{Limitations}
\begin{itemize}
    \item Question difficulty is confounded with modality in the unpaired comparison.
    \item The paired set is mostly tables and matrices with few real figures, so the causal result is really a table-reading penalty; geometric figures appear only in the confounded z-test.
    \item The text condition uses descriptions generated by Fable 5, so it partly tests the quality of those descriptions rather than the modality alone.
    \item The study covers a single course (02450) and a single model at one size (Gemma-4-E2B), so the results do not generalise broadly.
    \item The contamination check is weakly powered (only two clean exams, a minimum detectable effect of about 24\%), so the non-significant result is a non-rejection rather than proof.
    \item The 432 questions are treated as independent, but questions within one exam may cluster by that exam's difficulty. The McNemar test is unaffected by this, but it can affect the unpaired z-test and the binomial tests.
\end{itemize}

\subsection{Practical takeaway}
Locally run open multimodal models handle text multiple-choice questions well but are markedly weaker on figures and tables presented as images. This is relevant both for students relying on local models and for assessment design, since figure- and table-based questions are inherently more resistant to AI.
\section{Conclusion}
