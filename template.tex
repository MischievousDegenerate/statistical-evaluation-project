...
$$(p_1 - p_2)^2 = \frac{(Z_{1-\alpha/2} + Z_{1-\beta})^2 \left[ p_1(1 - p_1) + p_2(1 - p_2) \right]}{n}$$

and taking the square root of both sides gives

$$p_1 - p_2 = (Z_{1-\alpha/2} + Z_{1-\beta})\sqrt{\frac{p_1(1 - p_1) + p_2(1 - p_2)}{n}}$$

To find the smallest effect size detectable we set each term $p(1-p)$ at its maximum $0.25$, which occurs at $p = 0.5$. With $\alpha = 0.05$ (so $Z_{1-\alpha/2} = 1.96$), power $0.80$ (so $Z_{1-\beta} = 0.84$) and the smaller group size $n = 134$,
