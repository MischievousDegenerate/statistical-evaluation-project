...
The same method gives the minimum detectable effect for the contamination check. That test is one-sided (so $Z_{\alpha} = 1.645$) and its smaller group is the $n = 68$ post-cutoff 2025 questions, so
$$p_1 - p_2 = (1.645 + 0.84)\sqrt{\frac{0.25 + 0.25}{68}} \approx 0.21.$$
With only 68 post-cutoff questions the check is therefore only well-powered to detect a large pre-cutoff inflation (about $21$\% or more). The difference actually observed ($52.8\%$ vs.\ $67.6\%$) is smaller than this and runs the opposite way, so the non-significant result should be read with this limited power in mind.
...
Pure screenshot        & 36.1\% & $2.0 \times 10^{-3}$  \\
Paired text            & 63.0\% & $1.9 \times 10^{-19}$ \\
...
Pure Screenshot        & 36.1\% & $> 0.99$             & No  \\
Paired text            & 63.0\% & $1.7 \times 10^{-3}$ & Yes \\
