...
A possible problem might occur in that some of the binomial test are dependent. If the accuracy for an input modality is high, the full exam simulation will also be high. This could be a problem since according the slides day 4 (INSERT CITATION) bonferroni assumes independence of the data. This independence is neccessary if you want the family-wise error rate to be equal to $\alpha$, $FWER=\alpha$. Since our binomial tests aren't fully independent, the FWER wont be exactly $\alpha$, but it is known that it will be $FWER\le\alpha$. This means that we wont know our exact FWER, but since FWER is defined as the probability of getting one or more type 1 error, FWER decreased as dependence increased. The FWER therefore becomes $FWER\le\alpha$ increasing the probability of getting type 1 errors. In this report type 2 errors are preferable to type 1 errors. A type 1 error corresponds to telling the machine learning guys that they should change the exam format because there are problems with the current one, even if there isn't. A type 2 error corresponds to telling them to do nothing, when there should be done something. A type 1 error is therefore clearly worse than a type 2 error.
