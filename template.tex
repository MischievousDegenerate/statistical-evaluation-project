...
Running the model locally also made it straightforward to ensure statistical independence between observations, as each question could be sent in a completely fresh session with no shared state between calls\supercite{due2024evaluation}.\newline
...
Three different statistical frameworks were used to analyze the results\supercite{das2026slides}.
...
Since the dataset is fixed and numbers of questions can't be increased, the minimum effect size detectable by the z-test at 80\% power and $\alpha = 0.05$ is now computed\supercite{due2024evaluation}.
