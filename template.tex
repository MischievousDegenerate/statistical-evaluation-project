...
These minimum detectable effects were computed directly in Python with \texttt{numpy}, using the \texttt{numpy.sqrt} function.
...
For the contamination check, the one-sided test compared pre-cutoff (2017--2024) accuracy against the two 2025 exams, counting each question once. The result runs opposite to what contamination would predict regardless of how the paired questions are scored: Gemma was more accurate on the clean 2025 exams than on the older ones both when the paired questions are shown as images (66.7\% vs.\ 49.5\%) and as text (68.5\% vs.\ 56.1\%). Since the clean exams scored higher either way, there is no evidence that training-data memorisation is inflating the results.


\section{Discussion}
However, since the two groups consist of different questions, question difficulty is confounded with modality, so this comparison cannot tell us whether the image rendering itself causes the gap; that is what the paired McNemar test addresses.



\subsection{Interpretation of the results}
The clearest result in this study is the paired McNemar test. The same questions were shown to Gemma in two ways: once with the figure or table written out as text, and once as a screenshot. Gemma answered 64.2\% of these correctly when the information was given as text, but only 44.8\% when it was given as an image ($p = 8.9 \times 10^{-5}$), a drop of about 19 percentage points. Because the question is exactly the same in both cases and only the format changes, this gap cannot be explained by some questions simply being harder than others. It is the image format itself that makes Gemma worse, and this is the clearest evidence for the research question.

At the same time, Gemma is clearly not just guessing. In every format it scored well above the 25\% you would expect from random guessing, so it does read the questions and get real information out of them. The formats only split apart at the 50\% passing mark: Gemma passes when the information is given as text (the unpaired text questions, and the paired questions shown as text) but fails when the same information is shown as an image. Whether Gemma would pass a full exam therefore depends a lot on how many of the questions rely on figures and tables.

The unpaired comparison between the unpaired text questions (73.8\%) and the unpaired image questions (37.6\%) points the same way, but it is weaker evidence. These are different questions, so it is not clear whether the gap comes from the image format or just from those questions being harder. That is why the McNemar test is the main result and the z-test only backs it up.

A further concern was that Gemma might be remembering exams it had seen during training, since most of them are older than its knowledge cutoff. The contamination check shows this is not the case. Gemma actually did a little better on the two clean 2025 exams than on the older ones, which is the opposite of what would be expected if it was relying on memorised answers. This increases confidence that the format effect is real and not just a side effect of the training data.

It is worth asking why the image format hurts so much. Since Gemma answers correctly when the same numbers and labels are given as text, the problem does not seem to be its machine learning knowledge or its ability to reason about the question. The weak point is reading the information out of the image in the first place. In other words, the bottleneck is visual perception rather than reasoning: once the model is handed the numbers it can use them, but a small model like Gemma-4-E2B often fails to pull those numbers out of a figure or table correctly. This also fits the wider point that multimodal systems are still under-evaluated and tend to be weaker on non-text inputs.

\subsection{Limitations}
The study has several limitations. The most important one is that the unpaired text-versus-image comparison mixes two things together: the format and how hard the questions are. Only the paired McNemar test really separates them. On top of that, the paired set is mostly tables and matrices and contains only a few real figures, so strictly speaking the main result is about reading tables as images rather than figures in general. The geometric figures, like scatter plots and dendrograms, only appear in the weaker, confounded z-test.

Another limitation is that the text versions of the figures and tables were generated automatically by another model (Claude's Fable 5) rather than written by hand. So part of Gemma's higher score on the text version could come from a clear, easy-to-read description, and not only from text being easier than an image. The study also uses a single course (02450) and a single model at one size (Gemma-4-E2B), so the same results cannot be assumed to hold for other subjects, other models, or larger versions of Gemma. In particular, larger or cloud-based models with stronger vision could close the gap, so the weakness found here is specific to a small model that can be run locally.

Finally, the contamination check is based on only two clean exams, so it is not very powerful. With this little data it could only have detected a fairly large difference (around 24 percentage points), so the non-significant result does not prove there is no contamination, it only means none was found. All 432 questions are also treated as independent. This is reasonable for the model, since each question was answered in a fresh session with no shared memory, but questions from the same exam could still be linked through that exam being easy or hard. This does not affect the McNemar test, but it could have a small effect on the z-test and the binomial tests.

\subsection{Implications for exam design}
These results have a practical use for how universities design exams, especially exams taken at home or that are otherwise hard to supervise. A locally run open model is good at plain text multiple-choice questions, so an exam made only of text questions is easy to answer with free AI that any student can run on a normal laptop. Questions that depend on reading a figure, a plot, a diagram, or a table are much harder for such a model, so including more of these makes an exam more resistant to this kind of AI use. In practice an exam can be made harder to cheat on not by changing the topic, but simply by presenting the information visually instead of as text.

This protection is not complete. It is strongest against small, free, locally-run models like the one tested here; a student using a larger cloud model with better vision might still answer figure questions correctly. Figure- and table-based questions therefore raise the bar rather than fully closing the door. They are also a moving target: as open multimodal models keep getting better at reading images, this gap is likely to shrink, so exam design cannot rely on it forever. For now, leaning on visual questions is a simple and cheap way to make assessments more robust against the locally-run models that are currently within easy reach of students.
\section{Conclusion}

This study tested whether the open model Gemma-4-E2B does worse on machine learning exam questions when a figure or table is shown as an image instead of as text. The answer is yes. On the same questions, Gemma dropped from 64.2\% correct with a text description to 44.8\% with an image, a significant drop of about 19 percentage points. It still beats random guessing in every format, but it only passes the 50\% mark when the information is given as text. A contamination check found no sign that Gemma was simply remembering old exams, since it did slightly better on the newest exams that it cannot have seen. A locally run open model like Gemma is therefore genuinely useful for text questions but clearly struggles with figures and tables presented as images. For universities, this points to a simple way to make take-home exams more resistant to AI: rely more on questions that require reading a figure or a table.
