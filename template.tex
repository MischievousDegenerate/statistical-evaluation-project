...
\begin{itemize}
    \item \textbf{Pure questions:}
    \begin{itemize}
    \item \textbf{Text}: All information required to answer the question is presented as text.
    \item \textbf{Image}: The information required to answer the question is presented exclusively through a visual element such as a graph, figure, or diagram.
    \end{itemize}
    
    \item \textbf{Unpure questions}: The information required to answer the questions isquestion contains both visual representations and text.
\end{itemize}
...
\begin{itemize}
    \item Pure questions:
    \begin{itemize}
        \item \textbf{Unpaired text:} Gemma received the information text and answer options.
        \item \textbf{Unpaired image:} Gemma received the visual element as a screenshot together with the question text and answer options.
    \end{itemize}
    \item Unpure question:
    \begin{itemize}
        \item \textbf{Paired text}: The visual element was first converted into a detailed textual description using Claude's Fable 5. Gemma then received this generated description together with the original question text and answer options.
...
Pure examination questions can be considered independent, as each question is evaluated separately and does not share observations with other questions. Consequently, they produce unpaired (independent) observations. In contrast, the unpure question is evaluated under both prompt modalities, resulting in paired observations. This pairing allows for a direct comparison between modalities using McNemar’s test as further explained in section 2.4.1.
...
Ten binomial tests were conducted in this study. 4 to assess whether Gemma’s accuracy exceeded the 25\% baseline expected under random guessing in a four-option multiple-choice setting. 4 to assess whether Gemma could pass the exam i.e. a binomial test to see whether Gemma's accuracy exceeded 50\% for each modality. \newline 
The last 2 tests were full-exam simulations to see if Gemma could pass a realistic exam containing both text and image questions. The first one was a combination of Pure text, Pure image and the paired questions as text. The second one was a combination of Pure text, Pure image and the paired questions as a screenshot.\newline
all of these were one-sided tests since the purpose was to see how much better Gemma was than the baseline.
...
\begin{table}[H]
\centering
\caption{This table shows the results from the Gemma 4 run through the exam sets. Accuracy is calculated as $n_{correct}/n_{total}$.}
...
Unpaired text                         &  98 & 134 & 73.1\% & [65.1,\,79.9] \\
Unpaired image                   &  52 & 144 & 36.1\% & [28.7,\,44.2] \\
Paired text &  80 & 127 & 63.0\% & [54.3,\,70.9] \\
Paired image       &  54 & 127 & 42.5\% & [34.3,\,51.2] \\
\midrule
...
One-sided binomial tests were ran on each input format against two baselines: the 25\% accuracy expected from guessing ($H_0: p = 0.25$), and the 50\% DTU passing threshold ($H_0: p = 0.50$). All tests were compared to the bonferroni correct $\alpha_{\text{Bonf}}=0.0125$

\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Input format} & \textbf{Accuracy} & \textbf{$p$ (vs.\ 25\%)} & \textbf{$p$ (vs.\ 50\%)} & \textbf{Pass} \\
\midrule
Unpaired text         & 73.1\% & $2.0 \times 10^{-31}$ & $4.5 \times 10^{-8}$ & Yes \\
Unpaired image   & 36.1\% & $2.0 \times 10^{-3}$  & $> 0.99$             & No  \\
Paired text       & 63.0\% & $1.9 \times 10^{-19}$ & $1.7 \times 10^{-3}$ & Yes \\
Paired image & 42.5\% & $1.2 \times 10^{-5}$  & $0.95$               & No  \\
\bottomrule
\end{tabular}
\end{center}
...
Every modality beats chance, all four $p$-values against the 25\% baseline are far below the corrected threshold. Against the 50\% passing threshold the formats split along modalities. Gemma passes when the information is text (pure text and paired text descriptions) but fails when the questions are presented as a screenshot.
...
\textbf{Input format} & \textbf{Accuracy} & \textbf{$p$ (vs.\ 25\%)} & \textbf{$p$ (vs.\ 50\%)} & \textbf{Pass} \\
\midrule
Paired image  & 50.4\% & $5.6 \times 10^{-28}$ & $0.44$               & No  \\
Paired text   & 56.8\% & $4.8 \times 10^{-42}$ & $3.0 \times 10^{-3}$ & Yes \\
\bottomrule
\end{tabular}
\end{center}
...
When table questions are shown as screenshots, Gemma barely exceeds the passing threshold but the result is not statistically significant ($p = 0.44$). When tables are written out as text descriptions instead, Gemma passes with $p = 0.003$.
...
\subsection{Two-proportion z-tests: modality effect and contamination}

The two-proportion z-test was used for two comparisons: the pure-text vs.\ pure-image modality gap (two-sided), and the pre- vs.\ post-cutoff contamination check (one-sided).

\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Comparison} & \textbf{Accuracies} & \textbf{$z$} & \textbf{$p$} & \textbf{Significant} \\
\midrule
Pure text vs. pure image (two-sided) & 73.1\% vs. 36.1\% & $6.19$  & $6.1 \times 10^{-10}$ & Yes \\
Pre- vs. post-cutoff (one-sided)     & 52.8\% vs. 67.6\% & $-2.31$ & $0.99$                & No  \\
\bottomrule
\end{tabular}
\end{center}
...
Gemma was far more accurate on pure text questions (73.1\%, $n = 134$) than on geometric-figure screenshots (36.1\%, $n = 144$), a highly significant 37 percentage point gap. However, since the two groups consist of different questions, question difficulty is confounded with modality, so this comparison cannot tell us whether the image rendering itself causes the gap; that is what the paired McNemar test addresses.

For the contamination check, the one-sided test compared pre-cutoff (2017--2024) accuracy against the two 2025 exams. The result runs opposite to what contamination would predict: Gemma scored 67.6\% on the clean exams versus 52.8\% on the older ones, so there is no evidence that training-data memorisation is inflating the results.
