...
A possible problem might occur in that some of the binomial tests are dependent. If the accuracy for an input modality is high, the full exam simulation will also be high. This could be a problem since, according to the Day 2 slides\supercite{das2026day2}, Bonferroni assumes that the tests are independent. This independence is only needed if you want the family-wise error rate (FWER) to be exactly equal to $\alpha$, so $FWER = \alpha$. Since our binomial tests are not fully independent, the FWER will not be exactly $\alpha$, but it is still known that $FWER \le \alpha$ no matter how dependent the tests are. This means that we do not know our exact FWER. But since the FWER is the probability of getting one or more Type I errors, and dependence makes the Type I errors come in groups, so that either many happen at once or none happen, the FWER goes down as the dependence goes up. In other words, when the tests are dependent there are more scenarios where no Type I error happens than when they are independent. Applying Bonferroni therefore still keeps $FWER \le \alpha$, and the correction just becomes more conservative.

In this report a Type I error is worse than a Type II error. A Type I error would mean telling the course staff that figure and table questions are much more resistant to the model than they really are. This would give them a false sense of security and make them trust an exam format that does not actually stop students from using AI. A Type II error would only mean failing to spot a real difference between the modalities, which leaves the staff cautious but not misled. Acting on a false alarm is worse than failing to act on a real effect, so it makes sense to control the Type I errors with Bonferroni.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{modality.png}
    \caption{Accuracy across the four input modalities with 95\% confidence 
intervals. Blue bars indicate text-based modalities, red bars indicate 
image-based modalities. The dashed line marks the 25\% chance baseline 
and the dotted line marks the 50\% passing threshold.}
...
This pattern is consistent with the binomial results. Gemma exceeded the 50\% 
passing threshold for both unpaired and paired text respectively, but fell below it when 
the same information was given as an image (unpaired and paired). This suggests the bottleneck is not 
the model's understanding of machine learning content. When a table or matrix is 
written out as text, Gemma can generally reason about it correctly --- identifying 
relevant values, interpreting the structure, and selecting the right answer. The 
difficulty arises in the visual extraction step: reading numerical values off a 
screenshot appears to introduce errors that would not occur if those same values 
were provided directly as text.
...
The contamination check was included because most of the exam sets predate 
Gemma's training cutoff of January 2025, raising the possibility that the model 
had encountered some of these questions during training. The results do not 
support this concern: Gemma performed slightly better on the two 2025 exam sets 
it could not have seen than on the older material, which is the opposite of what 
would be expected if memorisation were inflating the scores. While this does not 
conclusively rule out contamination on individual questions, it suggests that 
the observed performance differences reflect genuine model behaviour rather than 
recall.


\subsection{Limitations}
The most significant limitation is one that runs through the whole study which is the 
unpaired text and image questions are not the same questions, so any difference 
between them could reflect difficulty as much as modality. The McNemar test 
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Unpaired question examples: an unpaired text question (left) and an unpaired image question (right).}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Paired question example (Spring~2017, Q25): the confusion matrix as
    shown to Gemma in the \textit{paired image} condition (left), and the same
    table written out by Claude's Fable~5 in the \textit{paired text} condition
    (right).}
