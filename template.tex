...
So with this fixed $n$ the test can detect a true accuracy gap of $17$\% at least $80\%$ of the time. The gap that was observed in the Gemma run between the pure text and pure image questions is $0.738 - 0.376 = 0.36$, which is a lot higher than $0.17$. This means that the z-test is a good test to use in this study, and that it will almost always find a statistical significant difference between $p_{text}$ and $p_{graph}$ if there is one.
...
Pure text + pure image + paired (image) & 223 & 432 & 51.6\% & [46.9,\,56.3] \\
Pure text + pure image + paired (text)  & 249 & 432 & 57.6\% & [53.0,\,62.3] \\
\midrule
...
\midrule
Unpaired text         & 73.8\% & $8.8 \times 10^{-34}$ & $7.3 \times 10^{-9}$ & Yes \\
Unpaired image   & 37.6\% & $3.3 \times 10^{-4}$  & $> 0.99$             & No  \\
Paired text       & 64.2\% & $1.4 \times 10^{-21}$ & $6.5 \times 10^{-4}$ & Yes \\
Paired image & 44.8\% & $5.1 \times 10^{-7}$  & $0.90$               & No  \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
Pure text + Pure image + Paired image  & 51.6\% & $2.1 \times 10^{-32}$ & $0.27$               & No  \\
Pure text + Pure image + Paired text   & 57.6\% & $6.0 \times 10^{-47}$ & $8.7 \times 10^{-4}$ & Yes \\
\bottomrule
\end{tabular}
\end{center}
...
When table questions are shown as screenshots, Gemma barely exceeds the passing threshold but the result is not statistically significant ($p = 0.27$). When tables are written out as text descriptions instead, Gemma passes with $p < 0.001$.
...
\midrule
Pure text vs. pure image (two-sided) & 73.8\% vs. 37.6\% & $6.26$  & $3.7 \times 10^{-10}$ & Yes \\
