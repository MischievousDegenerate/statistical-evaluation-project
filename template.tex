...
For the contamination check, a one-sided test compared pre-cutoff (2017--2024) accuracy against post-2025 accuracy, with each question counted once. The results run counter to what would be expected under data contamination, regardless of how the paired questions are prompted. Gemma achieved higher accuracy on the post-2025 exams than on the earlier exams both when the paired questions are presented as images (66.7\% vs.\ 49.5\%) and as text (68.5\% vs.\ 56.1\%). Since performance is consistently higher on the 2025 exams in both conditions, there is no evidence that training-data memorization is inflating the results.
...
The McNemar test is the result that matters most here, and the reason is simple. 
It is the only comparison in this study where question difficulty is not in play. 
The same 134 questions were answered in both formats, so the 19 percentage point 
drop from text to image cannot be explained by figure questions simply being 
harder. The format itself is what causes the difference. That also makes it easier to read the binomial results. Gemma passes the 50\% 
threshold when information is given as text, and fails when it is shown as an 
image. The natural question is why, and the answer does not seem to be that 
Gemma lacks machine learning knowledge. When you give it the same table written 
out as text it can usually reason about the content correctly. It knows what a 
correlation matrix is, it can identify the right value and it can follow the logic 
of the question. The problem is extracting that information from an image. A 
model this size running locally will often misread numbers, confuse rows, or 
lose precision when it has to pull values off a screenshot rather than receive 
them as text.

The unpaired z-test comparing text questions (73.8\%) to image questions (37.6\%) 
looks significant, but it is harder to interpret because these are different 
questions. Figure-based questions tend to be harder regardless of how they are 
presented, so some of that gap is probably difficulty rather than modality. The 
McNemar result is more convincing precisely because it controls for this.

The contamination check was worth running given that most of the exams predate 
Gemma's training cutoff. If the model had seen these questions before, you would 
expect it to score higher on the older material. Instead it scored slightly 
better on the two 2025 exams it could not have seen which is the wrong direction for 
contamination. That does not rule it out on individual questions, but at the 
level of overall performance it is not something to worry about.
...
The introduction to this paper cited a study finding that 9\% of undergraduate 
students admit to using AI to cheat\supercite{chirikov2026genai}. The results 
...
The main finding is straightforward. Gemma-4-E2B is noticeably worse at exam 
questions when it has to look at a figure or table rather than read it as text. 
On the paired questions, where the question itself stays the same and only 
the format changes accuracy dropped from 64.2\% with a text description to 
44.8\% with a screenshot. That 19 percentage point gap is hard to explain away, 
and the McNemar test confirms it is not just noise.

What is perhaps more interesting is what this tells us about why the 
model struggles. It is not that Gemma does not understand machine learning. When 
you hand it the same numbers written out as text, it can usually do something 
sensible with them. The problem is reading those numbers off an image in the 
first place. That is a much more specific weakness than people tend to assume 
when they worry about AI and exams. The contamination check did not find any sign that Gemma was coasting on 
memorised answers from old exams. If anything, it did slightly better on the 
2025 exams it could not have seen during training, which is the wrong direction 
for contamination to show up. So the results are probably measuring genuine 
performance rather than recall.

Some red flags are worth keeping in mind. This is one model, one course, and the 
text descriptions of figures were written by another AI, not by hand, so 
Gemma may have had an unusually clean version to read. Larger models with better 
vision will likely close the gap over time. But for a model small enough to run 
on a student laptop, the weakness is real and fairly large. The practical upshot for exam design is simple: questions that require reading a 
figure or a table are considerably harder for this kind of model than plain text 
questions. That does not mean they are AI-proof, but they do raise the bar in a 
way that does not require redesigning the entire exam.
...
\begin{checklist}
\chk[] Rephrase or improve wording, grammar, or style.
\chk[] Summarize external sources or content.
\chk[] Check or enhance logical flow or structure of the report.
\chk[] Draft sections of the report.
\end{checklist}

\textbf{Research \& Information Gathering}
\begin{checklist}
\chk[x] Find primary or secondary sources (papers, articles, data).
\chk[] Serve as a primary source of information or data.
\chk[] Summarize or extract key points from sources.
...
\chk[] Identify potential limitations or challenges.
\end{checklist}

\textbf{Design of Experiments / Methodology}
\begin{checklist}
\chk[] Design experiments or workflows.
\chk[x] Optimize parameters or settings for experiments.
\chk[] Propose control conditions or comparison groups.
\chk[] Assist with experimental planning or sequencing of tasks.
\end{checklist}

\textbf{Computer Code / Programming}
\begin{checklist}
\chk[x] Debug code or identify errors.
\chk[] Automate repetitive coding tasks.
\chk[x] Explain programming concepts, algorithms, or logic.
\chk[] Optimize or refactor code for efficiency or readability.
\chk[] Generate code snippets or full programs.
\end{checklist}
...
\chk[x] Generate diagrams, schematics, or drawings.
...
\chk[] Generate tables or visual summaries of data.
\end{checklist}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.60\linewidth]{images/image11.png}
    \caption{Paired image example --- the confusion matrix as shown to Gemma
    in the \textit{paired image} condition.}
    \label{fig:typeCimage}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{images/image111.png}
    \caption{Paired text example --- the same question as above, but the 
    confusion matrix replaced by a textual description generated by Claude's 
    Fable 5, as used in the \textit{paired text} condition.}
    \label{fig:typeCtext}
\end{figure}
